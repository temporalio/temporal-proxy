...
func newTestReporter(t *testing.T, labels metrics.MetadataLabels) (*server.Reporter, *goprom.Registry) {
	t.Helper()
	f, reg := newTestFactory(t)
	return server.NewReporter(f.ForSubsystem("server"), labels), reg
}
...
func TestReporterCarriesConfiguredMetadataLabels(t *testing.T) {
	t.Parallel()

	labels := metrics.NewMetadataLabels([]config.MetricLabel{{Header: "X-Tenant", Name: "tenant"}})
	r, reg := newTestReporter(t, labels)
...
func TestReporterInterceptorReadsMetadataLabelsFromTheStream(t *testing.T) {
	t.Parallel()
