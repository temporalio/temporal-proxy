...
// compatibility alias (see Expand): the full set a name might need to be
// checked against, as opposed to Known's plain spelling of it. A caller
// checking membership against "the complete set of names that count as
// forwardable" should use All rather than calling Known and Expand itself.
func All() []string {
	return Expand(Known())
}
