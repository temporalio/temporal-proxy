...
func TestReporterMetadataLabels(t *testing.T) {
	t.Parallel()

	labels := metrics.NewMetadataLabels([]config.MetricLabel{{Header: "X-Tenant", Name: "tenant"}})

	t.Run("pre-resolves nothing, since a label value arrives with a request", func(t *testing.T) {
		t.Parallel()

		_, reg := newLabeledTestReporter(t, labels, "primary")

		// The counterpart to "pre-resolves meaningful series to zero": a handle
		// would have to pin a label value, so none is taken and no series exists
	...
	t.Run("records the label on both collectors", func(t *testing.T) {
		t.Parallel()

		m, reg := newLabeledTestReporter(t, labels, "primary")
